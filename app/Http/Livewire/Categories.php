...
class Categories extends Component
{
    ...
    public function render()
    {
        if(strlen($this->search) > 0)   //validar si la caja de busqueda tiene algo escrito con metodo php strlen
            $data = Category::where('name', 'like', '%' . $this->search . '%')->paginate($this->pagination);    //busqueda por nombre
    ...
    public function Edit($id){  //metodo para abrir modal edit pasandole el id del elemento seleccionado
        ...
        $record = Category::find($id, ['id', 'name']); //obtener la informacion especifica del elemento seleccionado y almacenarlo en variable
    ...
    public function Update(){   //metodo para actualizar registro
        ...
        $rules = [  //reglas de validacion para cada campo en especifico
          ...
          'name' => "required|min:3|max:100|unique:categories,name,{$this->selected_id}"
        ];
    ...
    public function Destroy(Category $category){    //metodo para eliminar registros con una instancia del modelo que contiene el id
