<div class="row sales layout-top-spacing">
    <div class="col-sm-12">
        <div class="widget widget-chart-one">
            <div class="widget-heading">
                <h4 class="card-title text-center"><b>Informe de Egresos</b></h4>
            </div>
                        ...
                        <table class="table table-bordered table-striped mt-1">
                            ...
                            <tbody> <!--cuerpo de tabla-->
                                @if ($total <= 0)   <!--validar si existen ventas en las fechas seleccionadas-->
                                    <tr><td colspan="5"><h5 class="text-center text-muted">No hay ventas en la fecha seleccionada</h5></td></tr>
                                @endif

                                @if($userid > 0 && $fromDate != null && $toDate != null)    <!--validar que se selecciono un usuario,fecha inicial y fecha final-->

                                    @foreach ($sales as $sale)   <!--iteracion de los datos almacenados en variable pasada desde controlador-->
                                        <tr>
                                            <td class="text-center"><h6>{{$sale->product}}</h6></td>
                                            <td class="text-center"><h6>{{$sale->office}}</h6></td>
                                            <td class="text-center"><h6>{{$sale->items}}</h6></td>
                                            <!--funcion number_format de php para dar formato decimal recibe 2 parametros (numero,cantidad de decimales)-->
                                            <td class="text-center"><h6>${{number_format($sale->total, 2)}}</h6></td>
                                            <td class="text-center"><h6>{{$sale->user}}</h6></td>
                                            ...
                                            <td class="text-center">
                                                ...
                                                @can('Anular_Venta')    <!--aplicar permiso al boton para anular venta-->
                                                <a href="javascript:void(0)" onclick="Confirm('{{$sale->id}}')" class="btn btn-dark" title="Delete">
    ...
    @include('livewire.cashouts.modalDetails')   <!--modal de detalle de venta-->
